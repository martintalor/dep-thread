...
public class PkgGetter {
	...
	private ExecutorService fixedUpStreamThreadPool = Executors.newFixedThreadPool(1,
			new PkgGetter.DepThreadFactory("PkgGetter"));
	 ...
	 * @param jobId
	 */
	public void down(final String jobId, SectionNode sectionNode, ConcurrentHashMap<String, Object> outParam) {
...
//
//					try {
////						sectionUtils.insertSectionStepRecorders(param, new BigDecimal(0), sectionNodeInner.getCurrent().getClass() + "", jobId, totalSectionNumber,new BigDecimal(0));
...
////							return;
////						}
////						sectionUtils.insertSectionStepRecorders(result.getMap(), new BigDecimal(1), sectionNodeInner.getCurrent().getClass() + "", jobId, totalSectionNumber,new BigDecimal(0));
//
////						param = result.getMap();
//					} catch (Exception e) {
